    ...
    execute: (tx, ts, cb) => {
        ...
        cache.updateOne('accounts', 
            ...
            () => cache.findOne('accounts', {name: tx.sender}, function(err, accSender) {
                ...
                transaction.updateGrowInts(accSender, ts, () => transaction.adjustNodeAppr(accSender, -tx.data.amount, () => {
                    if (tx.data.receiver === config.burnAccount) return cb(true)
                    ...
                    cache.updateOne('accounts', 
                        ...
                        () => cache.findOne('accounts', {name: tx.data.receiver}, function(err, accReceiver) {
                            ...
                            transaction.updateGrowInts(accReceiver, ts, () => transaction.adjustNodeAppr(accReceiver, tx.data.amount, () => cb(true)))
                        }))
                }))
            }))
    }
