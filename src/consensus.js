...
var consensus = {
    ...
    finalizing: false,
    possBlocks: [],
    getActiveWitnessKey: (name) => {
        var shuffle = chain.schedule.shuffle
        for (let i = 0; i < shuffle.length; i++)
            if (shuffle[i].name === name)
                return shuffle[i].pub_witness
        return
    },
    isActive: () => {
        if (consensus.observer)
            return false
        var thPub = consensus.getActiveWitnessKey(process.env.NODE_OWNER)
        if (!thPub) {
        ...
        if (process.env.NODE_OWNER_PUB !== thPub) {
            consensus.observer = true
            logr.warn('Witness key does not match blockchain data, observing instead',thPub, process.env.NODE_OWNER_PUB)
            return false
        }
        return true
    },
    activeWitnesses: () => {
        ...
        var actives = []
        var currentWitness = chain.schedule.shuffle[(blockNum-1)%config.witnesses].name
        ...
        // logr.cons('Witnesses: ' + actives.join(','))
        return actives
    },
    tryNextStep: () => {
        var consensus_size = consensus.activeWitnesses().length
    ...
    endRound: (round, block) => {
        if (consensus.isActive()) {
            ...
            // only if we are an active witness
            var onlyBlockHash = {
                hash: block.hash
            }
    ...
    remoteRoundConfirm: (message) => {
        ...
        var witness = message.s.n
        
        for (let i = 0; i < consensus.possBlocks.length; i++) 
            if (block.hash === consensus.possBlocks[i].block.hash) {
                if (consensus.possBlocks[i][round] && consensus.possBlocks[i][round].indexOf(witness) === -1) {
                    // this witness has not already confirmed this round
                    //  add the witness to the ones who passed precommit
                    
                    for (let r = round; r >= 0; r--)
                        if (consensus.possBlocks[i][r].indexOf(witness) === -1)
                            consensus.possBlocks[i][r].push(witness)
    ...
    verifySignature: (message, cb) => {
        ...
        var pub = consensus.getActiveWitnessKey(name)
