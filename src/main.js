...
if (allowNodeV.indexOf(currentNodeV) === -1) {
    ...
    logr.warn('NodeJS v10 has reached end of life, hence v10 support for Breeze will be removed in the future. Please upgrade to the latest supported NodeJS v' + allowNodeV[allowNodeV.length])
...
function startRebuild(startBlock) {
    ...
    chain.rebuildState(startBlock,(e,headBlockNum) => {
        if (e) {
            ...
            logr.info('Rebuild interrupted, so far it took ' + (new Date().getTime() - rebuildStartTime) + ' ms. To resume, start Breeze with REBUILD_RESUME_BLK=' + headBlockNum)
        else
...
function startDaemon() {
    ...
    db.collection('blocks').findOne({_id: chain.getLatestBlock()._id - (chain.getLatestBlock()._id % config.witnesses)}, function(err, block) {
        if (err) throw err
